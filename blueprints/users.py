...
def profile() -> ResponseReturnValue:
    if request.method == 'POST':
        ...
        db = get_db()
        try:
            user = db.execute('SELECT * FROM users WHERE id = ?', (session[SESSION_USER_ID],)).fetchone()
            ...
            db.execute('UPDATE users SET password_hash = ? WHERE id = ?',
                       (generate_password_hash(new_password, method='pbkdf2:sha256'), session[SESSION_USER_ID]))
            db.commit()
        finally:
            db.close()
