...
def _to_rel_display(abs_path: str) -> Optional[str]:
    root = _storage_root()
...
def safe_join(relative: str) -> str:
...
def _list_directory(abs_dir: str) -> list[dict[str, object]]:
...
def _prune_missing_files(db: sqlite3.Connection) -> None:
...
def _remove_db_paths_for_prefix(db: sqlite3.Connection, prefix_abs: str) -> None:
...
def _update_db_paths_after_rename(
    db: sqlite3.Connection, old_abs: str, new_abs: str
) -> None:
    ...
    for row in rows:
        ...
        if fp == old_abs:
            ...
            db.execute(
                "UPDATE files SET filepath = ? WHERE id = ?", (new_fp, row["id"])
            )
...
def upload() -> ResponseReturnValue:
    ...
    if existing:
        db.execute(
            "UPDATE files SET size = ?, uploaded_by = ? WHERE filepath = ?",
...
def rename_item() -> ResponseReturnValue:
...
@permission_required("write")
def delete_item() -> ResponseReturnValue:
    rel = request.form.get("path", "").strip()
    parent_rel = request.form.get("parent_path", "").strip()
    try:
        target = safe_join(rel)
