...
class TestDownload:
    ...
    def test_download_traversal_blocked(self, admin_client):
        resp = admin_client.get('/files/download?path=../etc/passwd', follow_redirects=True)
        assert resp.status_code == 200
...
class TestRename:
    ...
    def test_rename_updates_db_filepath(self, admin_client, app):
        ...
        with app.app_context():
            from database import get_db
            db = get_db()
            row = db.execute("SELECT filename FROM files WHERE filename = 'renamed.txt'").fetchone()
            old = db.execute("SELECT filename FROM files WHERE filename = 'tracked.txt'").fetchone()
            db.close()
        assert row is not None
        assert old is None
...
class TestDelete:
    ...
    def test_delete_removes_db_record(self, admin_client, app):
        ...
        with app.app_context():
            from database import get_db
            db = get_db()
            row = db.execute("SELECT id FROM files WHERE filename = 'nodb.txt'").fetchone()
            db.close()
        assert row is None
...
class TestSecurity:
    ...
    def test_traversal_double_dot_blocked(self, admin_client):
        resp = admin_client.get('/files/?path=..%2F..%2Fetc%2Fpasswd', follow_redirects=True)
        assert b'Invalid path' in resp.data
