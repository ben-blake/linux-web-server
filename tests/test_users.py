...
def test_admin_can_edit_user(admin_client):
    ...
    resp = admin_client.post(
        ...
        data={"role": "admin", "permissions": "read,write,edit,admin"},
        follow_redirects=True,
    )
    assert resp.status_code == 200
...
def test_admin_can_delete_user(admin_client):
    ...
    db = get_db()
    user = db.execute(
        "SELECT id FROM users WHERE username = ?", ("deleteme",)
    ).fetchone()
    db.close()
...
def test_user_can_change_own_password(user_client):
    resp = user_client.post(
        "/users/profile",
        data={"current_password": "testpass", "new_password": "newpassword"},
        follow_redirects=True,
    )
    assert resp.status_code == 200
...
def test_profile_empty_new_password_rejected(user_client):
    resp = user_client.post(
        "/users/profile",
        data={"current_password": "testpass", "new_password": ""},
        follow_redirects=True,
    )
    assert resp.status_code == 200
...
def test_delete_user_nullifies_file_ownership(admin_client, app):
    ...
    with app.app_context():
        ...
        db = get_db()
        uploader = db.execute(
            "SELECT id FROM users WHERE username = 'uploader'"
        ).fetchone()
    ...
    with app.app_context():
        ...
        db = get_db()
        row = db.execute(
            "SELECT uploaded_by FROM files WHERE filename = 'owned.txt'"
        ).fetchone()
        db.close()
